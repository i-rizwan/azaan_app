package com.infigo.alshamsazaan.ui.main.quran.readsurah
...
import com.infigo.alshamsazaan.databinding.ListReadSurahBinding
...
class ReadSurahAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ReadSurahViewHolder {
        val binding =
            ListReadSurahBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return ReadSurahViewHolder(binding)
    }

    override fun getItemCount(): Int = listAyah.size

    override fun onBindViewHolder(holder: ReadSurahViewHolder, position: Int) =
        holder.bind(listAyah[position])

    inner class ReadSurahViewHolder(private val binding: ListReadSurahBinding) :
